...
// STARS log level API/resource
	.factory('LogLevelAPI', [
		'$resource',
		function ($resource) {
			return $resource('/api/v2/STARS_LogLevel', {}, {
				query: {
					method: 'GET',
					isArray: false
				},
...
.controller('LogViewerCtrl2', [
	...
	function ($scope, LogEntries, LogLevelAPI, $location, $route, $mdToast, $q, $filter, $interval) {
		...
		$scope.StarsLogLevel = "(unknown)";

		$scope.loadLogLevel = function() {
			LogLevelAPI.query().$promise.then(function (result) {
				alert(result);
				console.log(result);
			})
		};
